...
export function BalanceSummary({ income, expenses, balance, formatCurrency }: BalanceSummaryProps) {
  ...
  return (
    <section 
      className="mb-6 bg-card rounded-lg p-4 shadow-sm border-2"
      style={{ borderColor: 'hsl(var(--balance-border))' }}
    >
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-4 text-center">
        <div>
          ...
          <p className="text-green-600 font-bold text-base sm:text-lg">{formatCurrency(income)}</p>
        </div>
        <div>
          <p className="text-muted-foreground text-xs sm:text-sm">Gastos</p>
          <p className="text-red-600 font-bold text-base sm:text-lg">{formatCurrency(expenses)}</p>
        </div>
        <div>
          <p className="text-muted-foreground text-xs sm:text-sm">Balance</p>
          <p className={`font-bold text-base sm:text-lg ${balanceColor}`}>{formatCurrency(balance)}</p>
        </div>
      </div>
    </section>
  );
